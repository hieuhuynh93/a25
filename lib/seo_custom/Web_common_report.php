...
class Web_common_report {
    ...
    function google_index($url) {
        ...
        preg_match('#<div.*?id="resultStats".*?>.*?([\d,]+).*?results</div>#si', $response, $google_index);
        $g_index = '';
        if (!empty($google_index)) {
            $g_index = $google_index[1];
        } else {
            preg_match('#<div.*?id="resultStats".*?>.*?([\d,]+).*?result</div>#si', $response, $google_index);
            if (!empty($google_index)) {
                $g_index = $google_index[1];
            } else {
                $g_index = 0;
...
// keyword research
    ...
    function link_statistics_url($url) {
        ...
        foreach ($links[1] as $link_info) {
            ...
            /*             * ***** */
            if ($link == "" || substr($link, 0, 1) == '#' || stripos($link, "javascript:") !== FALSE || stripos($link, "tel:") !== FALSE || stripos($link, "mailto:") !== FALSE || stripos($link, "/auth") !== FALSE) {
                continue;
            }
